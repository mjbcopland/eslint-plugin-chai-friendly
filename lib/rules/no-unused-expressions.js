...
module.exports = {
    ...
    create: function (context) {
        ...
        function isChaiCall(node) {
            return isChaiExpectCall(node) || isChaiShouldCall(node);
        }

        function isWhitelistCall(node) {
            var whitelist = [
                'ok',
                'true',
                'false',
            ...
            var isOnWhitelist = whitelist.indexOf(node.expression.property.name) !== -1;
        ...
        return {
            ExpressionStatement: function(node) {
                var valid = isValidExpression(node.expression)
                    ...
                    || (isChaiCall(node) && isWhitelistCall(node));
                if (!valid) {
